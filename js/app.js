...
function startQR() {
  ...
  qrScanner.start(
    ...
    function(decodedText) { stopQR(); handleQRResult(decodedText); },
    function(err) {}
  ).catch(function(err) {
...
function handleQRResult(text) {
  ...
  if (email && DEMO_PURCHASES[email]) {
    ...
    setTimeout(function() { window.location.href = 'pages/appliances.html'; }, 800);
  } else {
...
function lookupByEmail() {
  var rawEmail = document.getElementById('email-input').value;
  ...
  // Check if DEMO_PURCHASES loaded correctly
  if (typeof DEMO_PURCHASES === 'undefined') {
    errorDiv.textContent = 'App data not loaded. Please refresh the page and try again.';
    errorDiv.classList.remove('hidden');
    return;
  }
  ...
  if (DEMO_PURCHASES[email]) {
    ...
    errorDiv.textContent = 'No purchase found for "' + email + '". Try demo@homeguide.nz';
    errorDiv.classList.remove('hidden');
  }
}
